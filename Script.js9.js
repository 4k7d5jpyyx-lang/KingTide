(() => {
  ...
  function startBossCameraPull(x, y) {
    bossCam = {
      ...
      startY: camY,
      startZoom: zoom,
      targetZoom: clamp(zoom * 1.18, 0.6, 1.9),
      prevFocus: focusOn
    };
    focusOn = false;
  }
  ...
  function checkMilestones() {
    ...
    for (const m of BOSS_MILESTONES) {
      ...
      if (mcap >= m.at) {
        ...
        // cinematic camera pull on boss spawn
        if (boss.segs && boss.segs[0]) startBossCameraPull(boss.segs[0].x, boss.segs[0].y);
        c.worms.push(boss);
  ...
  function step(dt, time) {
    ...
    // Boss cinematic camera override (temporary)
    if (bossCam) {
      bossCam.t += dt;
      const p = clamp(bossCam.t / bossCam.dur, 0, 1);
      const e = p * p * (3 - 2 * p); // smoothstep

      camX = lerp(bossCam.startX, -bossCam.x, e);
      camY = lerp(bossCam.startY, -bossCam.y, e);
      zoom = lerp(bossCam.startZoom, bossCam.targetZoom, e);
