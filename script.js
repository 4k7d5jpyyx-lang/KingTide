...
// =========================
// Boss Cinematic Camera Pull
// =========================
let __bossCamT = 0;
let __bossCam = null;

function startBossCinematic(x, y) {
  __bossCam = { x, y };
  __bossCamT = performance.now() + 1400; // 1.4s cinematic
}

function applyBossCinematic(dt) {
  if (!__bossCam || performance.now() > __bossCamT) return;
  const t = (__bossCamT - performance.now()) / 1400;
  const ease = t * t;
  cam.x += ( __bossCam.x - cam.x) * 0.06 * ease;
  cam.y += ( __bossCam.y - cam.y) * 0.06 * ease;
  zoom = Math.min(zoom * 1.002, 1.35);
}
